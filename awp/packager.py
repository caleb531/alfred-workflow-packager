    ...
    with open(plist_path, "rb") as plist_file:
        ...
        # properly parse out the valid XML
            ...
            plist_contents = plist_contents[
                : plist_contents.index(junk_marker) + len(junk_marker)
            ]
...
def resources_are_equal(resource_path, dest_resource_path):
    ...
    try:
        ...
        try:
            return filecmp.cmp(resource_path, dest_resource_path) and cmp_permissions(
                resource_path, dest_resource_path
            )
        except OSError:
...
def copy_pkg_resources(workflow_path, workflow_resources, force=False):
    ...
    for resource_patt in workflow_resources:
        for resource_path in glob.iglob(resource_patt):
            ...
            copied = copy_resource(resource_path, dest_resource_path, force=force)
            if copied:
                copied_any = True
    if not copied_any:
...
def zip_workflow_dirs(workflow_path, zip_file):
    ...
    for root, dirs, files in os.walk(workflow_path):
        ...
        zip_workflow_dir_files(workflow_path, zip_file, root, relative_root, files)
...
def package_workflow(config, version, export_files, force=False):
    ...
    for export_file in export_files:
        ...
        export_workflow(workflow_path, os.path.join(project_path, export_file))
        print(
            "Exported v{version} to {file}".format(
